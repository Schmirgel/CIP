...
tokens{VAR;ANWEISUNG;WERTZUWEISUNG;IFTHENELSE;WHILE;PRINT;READ;}


start
	: START_TOKEN deklaration* BEGIN_TOKEN anweisung+ END_TOKEN -> deklaration* ^(ANWEISUNG anweisung+);
	
deklaration
	: konstanten IDENTIFIER (',' IDENTIFIER)* ';' -> ^(VAR konstanten IDENTIFIER); //VAR ist Elternknoten, alles andere sind Kinder
...
atom	
	...
	-> ^(IFTHENELSE ^('if' vergleich) ^('then' anweisung) ^('else' anweisung+)); //fi nicht mehr in der Anzeige
...
elihw
	...
	-> ^(WHILE ^('while' vergleich) ^('do' anweisung+)); //od nicht mehr in der Anzeige
